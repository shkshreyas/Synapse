...
import { InteractiveKnowledgeGraph, GraphNode, GraphEdge, GraphCluster } from './knowledgeGraph';
...
/**
 * Dashboard controller for MindScribe Chrome extension
...
class DashboardController {
    ...
    private initializeDOMElements(): void {
        this.globalSearch = document.getElementById('globalSearch') as HTMLInputElement;
        ...
        this.sortFilter = document.getElementById('sortFilter') as HTMLSelectElement;
    ...
    private setupEventHandlers(): void {
        ...
        document.querySelectorAll('.view-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const view = (e.target as HTMLElement).dataset.view as 'list' | 'grid' | 'graph';
                if (view) {
        ...
        if (analyticsBtn) {
            analyticsBtn.addEventListener('click', () => this.switchView('analytics'));
        }
        ...
        if (executeBulkActionBtn) {
            executeBulkActionBtn.addEventListener('click', () => this.showBulkActionModal());
        }
        ...
        if (zoomInBtn) zoomInBtn.addEventListener('click', () => this.handleGraphZoom('in'));
        if (zoomOutBtn) zoomOutBtn.addEventListener('click', () => this.handleGraphZoom('out'));
        if (resetViewBtn) resetViewBtn.addEventListener('click', () => this.handleGraphReset());
    }
        ...
        this.searchTimeout = window.setTimeout(async () => {
            ...
            try {
                ...
                if (searchResult.success && searchResult.semanticMatches) {
                    ...
                    const matchedIds = new Set(searchResult.semanticMatches.map(m => m.contentId));
                    ...
                    const scoreMap = new Map(
                        searchResult.semanticMatches.map(m => [m.contentId, m.combinedScore])
                    );
    ...
    private applyFilters(): void {
        ...
        if (this.typeFilter?.value) {
            filtered = filtered.filter(content =>
                content.metadata.pageType === this.typeFilter!.value
            );
        }
        ...
        if (this.timeFilter?.value) {
            ...
            filtered = filtered.filter(content =>
                new Date(content.timestamp) >= filterDate
            );
        }
    ...
    private createListItem(content: StoredContent): HTMLElement {
        ...
        const summary = content.summaries?.quick ||
    ...
    private createGridCard(content: StoredContent): HTMLElement {
        ...
        const summary = content.summaries?.tldr ||
    ...
    private async loadAnalyticsData(): Promise<void> {
        try {
            this.analyticsData = this.analyticsEngine.generateAnalytics(this.currentContent);
        } catch (error) {
            ...
            this.analyticsData = {
                ...
                accessPatterns: { mostAccessed: [], recentlyAccessed: [], neverAccessed: [] },
                contentGrowth: [],
    ...
    private updateAnalyticsDisplay(data: AnalyticsData): void {
        ...
        const mostActiveCountEl = document.getElementById('analyticsMostActiveCount');
        const topCategoryEl = document.getElementById('analyticsTopCategory');
        const topCategoryCountEl = document.getElementById('analyticsTopCategoryCount');
        ...
        if (mostActiveEl && data.mostActiveDay) {
            mostActiveEl.textContent = new Date(data.mostActiveDay.date).toLocaleDateString('en-US', { weekday: 'long' });
        }
    ...
    private renderAnalyticsCharts(data: AnalyticsData): void {
        ...
        if (chartContainer && data.captureActivity.length > 0) {
            ...
            chartRenderer.renderLineChart(data.captureActivity, 'Capture Activity (Last 30 Days)');
        }
    ...
    private async updateStats(): Promise<void> {
        ...
        const thisWeekItems = this.currentContent.filter(content =>
            new Date(content.timestamp) >= sevenDaysAgo
        ).length;

        if (this.totalItemsEl) this.totalItemsEl.textContent = totalItems.toString();
        ...
        if (confirm('Are you sure you want to delete this content?')) {
            try {
                ...
                if (result.success) {
                    ...
                    this.currentContent = this.currentContent.filter(c => c.id !== contentId);
                    this.filteredContent = this.filteredContent.filter(c => c.id !== contentId);
        ...
        try {
            const exportData = {
                ...
                content: this.currentContent.map(content => ({
                    ...
                    screenshot: content.screenshot ? '[Screenshot data removed]' : undefined
                }))
            };
    ...
    private updateSelectionUI(): void {
        ...
        document.querySelectorAll('.content-checkbox').forEach(checkbox => {
            ...
            if (contentId) {
                (checkbox as HTMLInputElement).checked = this.selectedContentIds.has(contentId);
            }
        });
    }

    private showBulkActionModal(): void {
        const bulkActionSelect = document.getElementById('bulkActionSelect') as HTMLSelectElement;
    ...
    private async executeBulkAction(actionType: string): Promise<void> {
        ...
        try {
            switch (actionType) {
                ...
                case 'updateCategory':
                    const newCategory = (document.getElementById('newCategory') as HTMLSelectElement)?.value;
                    if (!newCategory) throw new Error('Please select a category');
                    operation = { type: 'updateCategory', contentIds, parameters: { category: newCategory } };
                    break;

                case 'addTags':
                    const newTagsInput = (document.getElementById('newTags') as HTMLInputElement)?.value;
                    if (!newTagsInput) throw new Error('Please enter tags to add');
                    const newTags = newTagsInput.split(',').map(tag => tag.trim()).filter(tag => tag);
                    operation = { type: 'addTags', contentIds, parameters: { tags: newTags } };
                    break;

                case 'removeTags':
                    const removeTagsInput = (document.getElementById('tagsToRemove') as HTMLInputElement)?.value;
                    if (!removeTagsInput) throw new Error('Please enter tags to remove');
                    const tagsToRemove = removeTagsInput.split(',').map(tag => tag.trim()).filter(tag => tag);
                    operation = { type: 'removeTags', contentIds, parameters: { tags: tagsToRemove } };
                    break;

                case 'updateImportance':
                    const importance = parseInt((document.getElementById('importanceLevel') as HTMLInputElement)?.value || '5');
                    operation = { type: 'updateImportance', contentIds, parameters: { importance } };
                    break;
            ...
            // Show results
            if (result.failed > 0) {
                alert(`Operation completed with ${result.success} successes and ${result.failed} failures.\n\nErrors:\n${result.errors.join('\n')}`);
            } else {
                alert(`Operation completed successfully! ${result.success} items processed.`);
            }
    ...
    private async exportSelectedContent(contentIds: string[]): Promise<void> {
        ...
        const selectedContent = this.currentContent.filter(content => contentIds.includes(content.id));
        ...
        switch (format) {
            case 'json':
                ...
                filename = `mindscribe-export-${new Date().toISOString().split('T')[0]}.json`;
                mimeType = 'application/json';
                break;

            case 'markdown':
                exportData = selectedContent.map(content => `
# ${content.title}
...
---
`).join('\n');
                filename = `mindscribe-export-${new Date().toISOString().split('T')[0]}.md`;
                mimeType = 'text/markdown';
                break;

            case 'csv':
                const csvHeaders = ['Title', 'URL', 'Date', 'Category', 'Tags', 'Summary'];
                ...
                filename = `mindscribe-export-${new Date().toISOString().split('T')[0]}.csv`;
                mimeType = 'text/csv';
                break;
