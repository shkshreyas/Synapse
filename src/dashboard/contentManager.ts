...
export class ContentManager {
  ...
  async filterContent(
    ...
    if (filter.importance) {
      filtered = filtered.filter((item) => {
        ...
        return (
          importance >= filter.importance!.min &&
          importance <= filter.importance!.max
        );
      });
    }
    ...
    if (filter.language) {
      filtered = filtered.filter(
        (item) => item.metadata.language === filter.language
      );
    }
      ...
      try {
        switch (operation.type) {
          case "delete":
            ...
            break;
          case "updateCategory":
            await this.updateContentCategory(
              contentId,
          ...
          case "addTags":
            ...
            break;
          case "removeTags":
            await this.removeTagsFromContent(
              contentId,
              operation.parameters.tags
            );
            break;
          case "updateImportance":
            await this.updateContentImportance(
              contentId,
              operation.parameters.importance
            );
            break;
          case "export":
  ...
  async updateCollection(
    ...
    const collection = this.organization.collections.find(
      (c) => c.id === collectionId
    );
    if (!collection) {
  ...
  async deleteCollection(collectionId: string): Promise<void> {
    this.organization.collections = this.organization.collections.filter(
      (c) => c.id !== collectionId
    );
    await this.saveOrganization();
  }
  ...
  async addContentToCollection(
    ...
    const newIds = contentIds.filter(
      (id) => !collection.contentIds.includes(id)
    );
  ...
  async updateSmartFilter(
    filterId: string,
    updates: Partial<SmartFilter>
  ): Promise<void> {
    const filter = this.organization.smartFilters.find(
      (f) => f.id === filterId
    );
    if (!filter) {
  ...
  async deleteSmartFilter(filterId: string): Promise<void> {
    this.organization.smartFilters = this.organization.smartFilters.filter(
      (f) => f.id !== filterId
    );
    await this.saveOrganization();
  }
  ...
  async createCustomTag(
    ...
    const existing = this.organization.customTags.find(
  ...
  async deleteCustomTag(tagId: string): Promise<void> {
    this.organization.customTags = this.organization.customTags.filter(
      (t) => t.id !== tagId
    );
    await this.saveOrganization();
  }
  ...
  async generateContentStats(content: StoredContent[]): Promise<ContentStats> {
    ...
    stats.averageImportance =
      content.length > 0 ? totalImportance / content.length : 0;
  ...
  async suggestCollections(
    content: StoredContent[]
  ): Promise<{ name: string; description: string; contentIds: string[] }[]> {
    const suggestions: {
      name: string;
      description: string;
      contentIds: string[];
  ...
  private async updateContentCategory(
    contentId: string,
    category: string
  ): Promise<void> {
  ...
  private async addTagsToContent(
    ...
    const updateResult = await this.contentStore.update(contentId, {
      tags: updatedTags,
    });
    ...
    newTags.forEach((tagName) => {
      const customTag = this.organization.customTags.find(
        (t) => t.name === tagName
      );
      if (customTag) {
  ...
  private async removeTagsFromContent(
    contentId: string,
    tagsToRemove: string[]
  ): Promise<void> {
    const readResult = await this.contentStore.read(contentId);
  ...
  private async updateContentImportance(
    contentId: string,
    importance: number
  ): Promise<void> {
  ...
  private async loadOrganization(): Promise<void> {
    try {
      ...
      const stored = localStorage.getItem("synapse-organization");
      if (stored) {
