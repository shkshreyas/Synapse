...
class BackgroundService {
  ...
  private async handleGetRelatedContent(message: any): Promise<any> {
    ...
    return {
      content: [],
      message: 'Related content functionality will be implemented in later tasks'
    };
  }

  private async handleOpenSidePanel(sender: chrome.runtime.MessageSender): Promise<any> {
    if (sender.tab?.id) {
      try {
