// Full-text search indexing system for MindScribe

import { getDatabase } from './database';
import { StoredContent, SearchIndexEntry, SearchableMetadata } from '../types/storage';
...
export class SearchIndex {
    ...
    constructor() {
        ...
        this.stopWords = new Set([
            ...
            'also', 'am', 'another', 'any', 'because', 'before', 'being',
            'between', 'both', 'but', 'came', 'come', 'did', 'each', 'even',
            'every', 'get', 'got', 'had', 'has', 'have', 'having', 'here',
            'him', 'his', 'how', 'i', 'into', 'just', 'like', 'make', 'me',
            'most', 'my', 'no', 'now', 'only', 'or', 'other', 'our', 'over',
            'own', 'see', 'so', 'some', 'such', 'than', 'through', 'time',
            'two', 'very', 'way', 'we', 'well', 'what', 'when', 'where',
            'who', 'why', 'work', 'year', 'years'
        ]);
    }
    ...
    async indexContent(content: StoredContent): Promise<IndexingResult> {
        ...
        try {
            ...
            return {
                success: false,
                contentId: content.id,
                error: `Indexing error: ${error instanceof Error ? error.message : 'Unknown error'}`,
                operationTime: performance.now() - startTime
            };
        }
    }
    ...
    async removeFromIndex(contentId: string): Promise<IndexingResult> {
        ...
        try {
            ...
            return {
                success: false,
                contentId,
                error: `Remove error: ${error instanceof Error ? error.message : 'Unknown error'}`,
                operationTime: performance.now() - startTime
            };
        }
    }
    ...
    async search(query: string, options?: SearchOptions): Promise<SearchResult> {
        ...
        try {
            ...
            if (!validation.isValid) {
                return {
                    success: false,
                    error: `Invalid query: ${validation.errors.map(e => e.message).join(', ')}`,
                    results: [],
                    totalResults: 0,
                    operationTime: performance.now() - startTime
                };
            ...
            return new Promise((resolve) => {
                ...
                request.onsuccess = () => {
                    ...
                    const scoredResults = this.scoreSearchResults(allEntries, searchTerms, options);
    ...
    async rebuildIndex(): Promise<RebuildResult> {
        ...
        try {
            ...
            return {
                ...
                successCount: 0,
                failureCount: 0,
                errors: [`Rebuild failed: ${error instanceof Error ? error.message : 'Unknown error'}`],
                operationTime: performance.now() - startTime
            };
        }
    }
    ...
    private scoreSearchResults(
        ...
        return entries.map(entry => {
            ...
            for (const searchTerm of searchTerms) {
                const termMatches = entry.terms.filter(indexTerm =>
                ...
                if (termMatches > 0) {
                    ...
                    if (entry.terms.includes(`tag:${searchTerm}`) ||
            ...
            const daysSinceCreation = (Date.now() - new Date(entry.lastUpdated).getTime()) / (1000 * 60 * 60 * 24);
    ...
    private applyFilters(results: ScoredSearchResult[], filters: SearchFilters): ScoredSearchResult[] {
        return results.filter(result => {
            ...
            if (filters.minImportance &&
                (!result.metadata.importance || result.metadata.importance < filters.minImportance)) {
                return false;
            }
    ...
    private extractSearchableMetadata(content: StoredContent): SearchableMetadata {
        return {
